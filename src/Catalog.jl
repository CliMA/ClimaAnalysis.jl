...
function add_file!(catalog::NCCatalog, filepaths::Vector, short_names...)
    ...
    if isempty(maybe_short_name_pairs)
        ...
        short_name =
            maybe_short_name_pair isa Pair ? first(maybe_short_name_pair) :
            maybe_short_name_pair
    end

    time_dim_name = _get_time_name_from_mfds(filepaths; short_name)
...
"""
    _add_file!(catalog::NCCatalog, nc, maybe_short_name_pairs, filepaths)
