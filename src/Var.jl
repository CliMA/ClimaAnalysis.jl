...
shifting the longitudes. This is necessary to prevent duplicated latitudes after shifting
longitudes.
...
function shift_longitude(var, lower_lon, upper_lon)
    ...
    for i in eachindex(lon)
        if (lon[i] >= upper_lon) || (lon[i] < lower_lon)
            lon[i] = (lon[i] - lower_lon + width) % width + lower_lon
        end
    end
