...
Given `dim_name` in `var`, split the `OutputVar` by the values in `split_arrays`
and return a vector of `OutputVar`s.

For example, if `dim_name = "time" and `split_arrays = [[0.0, 3.0], [2.0,
...
function _split_along_dim(var::OutputVar, dim_name, split_vectors)
    # Split data by vectors in split_arrays
    split_data = (
        collect(
            _data_at_dim_vals(
                var.data,
