export flatten, unflatten, flatten_dim_order, flattened_length, arecompatible
...
Return whether two `FlatVar` are defined on the same physical space.
...
contain information about the units) and which values are dropped.
...
function arecompatible(x::Metadata, y::Metadata; ignore_dims = ())
    ...
    for x_dim_name in dim_names(x)
        ...
        dim_units(x, x_dim_name) == dim_units(y, y_dim_name) || return false
    end
