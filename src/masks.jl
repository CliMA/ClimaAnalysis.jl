...
After resampling, values greater than or equal to `threshold` are rounded to one and all
other values are rounded to zero. Zeros are replaced with `zero_to` and ones are replaced
with `one_to`.

The masking is applied as element wise multiplication.
"""
