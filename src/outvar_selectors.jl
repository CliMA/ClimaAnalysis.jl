export slice,
    window, select, NearestValue, MatchValue, Index, AbstractSelector, get_index
...
"""
function select(var::OutputVar; by = NearestValue(), kwargs...) # TODO: Make view_select too!
    dims, data = _select(var, by; kwargs...)
    return remake(var, dims = dims, data = reshape(copy(data), size(data)))
end
...
function _select_indices(
    ...
    # We cannot support `CartesianIndex` because of the line below. This is because it is
    # difficult to tell whether indices_or_vals is a Dates.DateTime or an iterable of
    # Dates.DateTime
    val_is_date =
