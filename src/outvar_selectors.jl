...
Get the index of the matched value.
...
- `MatchValue()`: The matched value will be selected for slicing.
...
!!! compat "Support for AbstractSelector"
    Calling `slice` with the `by` keyword argument was introduced in ClimaAnalysis v0.5.19.
...
- `MatchValue()`: The matched value will be selected for windowing.
...
Get the index that match exactly with `val` in the `dim_name` dimension.
